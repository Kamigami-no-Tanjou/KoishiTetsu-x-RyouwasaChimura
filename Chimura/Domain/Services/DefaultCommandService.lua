...
---
function DefaultCommandService.retrieveFromId(id)
    ...
    row.commandArgs = caSv.retrieveFromDefaultCommandId(tonumber(row.id))
...
---
function DefaultCommandService.retrieveDisabledDCFromGuildId(guildId)
    local disabledDCs = {}
...
--- @return void
---
function DefaultCommandService.linkToGuild(guildId)
    ...
    local result = assert(env.con:execute("SELECT ID FROM DefaultCommand;", "{ \"err\":\"Request failed!\" }"))
    ...
    while DCId do
        assert(env.con:execute(string.format(requestSkeleton, true, guildId, tonumber(DCId[1]))))
...
--- @return void
---
function DefaultCommandService.deleteAtGuildId(guildId)
    assert(guildId ~= nil and guildId > 0, "{ \"err\":\"Bad ID!\" }")
    local requestSkeleton = [[
